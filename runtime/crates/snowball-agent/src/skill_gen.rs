...
impl PermissionPolicy {
    pub fn authorize(&self, tool: &str) -> PermissionOutcome {
        ...
        if self.active_mode >= *required { Allow } else { Deny }
            ...
            // ── Gap-filling skills (from Python analysis) ───────────────

            GeneratedSkill {
                id: "skill_workflow_state".into(),
                name: "Implement Workflow State & Idempotency".into(),
                description: "Build workflow state management with immutable state objects, idempotency keys, and crash-safe checkpoints. Resuming a conversation is NOT resuming a workflow.".into(),
                category: PatternCategory::SessionManagement,
                trigger: "When your agent performs multi-step tasks with side effects that must not be duplicated on retry.".into(),
                steps: vec![
                    "Model work as explicit states: planned, awaiting_approval, executing, waiting_on_external, completed, failed.".into(),
                    "Use frozen/immutable dataclasses for state objects (transitions create new instances).".into(),
                    "Persist workflow checkpoints after every side-effecting step.".into(),
                    "Give mutating operations idempotency keys so retries don't double-fire.".into(),
                    "Separate conversation state (what we said) from task state (what step we're on).".into(),
                    "Implement replay() to reconstruct workflow from persisted checkpoints.".into(),
                ],
                verification: vec![
                    "Agent crash mid-task → resume picks up at correct step.".into(),
                    "Retry of completed step is a no-op (idempotent).".into(),
                    "Workflow state persisted after every side effect.".into(),
                    "Conversation transcript and workflow state are independent.".into(),
                ],
                code_template: String::new(),
                source_facts: vec!["py_workflow_state".into()],
            },

            GeneratedSkill {
                id: "skill_system_event_logging".into(),
                name: "Implement System Event Logging".into(),
                description: "Build a structured event log separate from conversation. Answers 'what did the system do?' vs 'what did we say?'.".into(),
                category: PatternCategory::AgenticLoop,
                trigger: "When you need to debug agent behavior after the fact, or audit system operations.".into(),
                steps: vec![
                    "Define HistoryEvent(title, detail) as immutable record.".into(),
                    "Create HistoryLog with append-only events list.".into(),
                    "Log at key points: context loading, registry init, routing decisions, execution, permission denials, session persistence.".into(),
                    "Add as_markdown() for human-readable rendering.".into(),
                    "Keep separate from conversation transcript.".into(),
                    "Optionally add event categories (enum) for filtering.".into(),
                ],
                verification: vec![
                    "System events captured for all key operations.".into(),
                    "Event log survives session persistence.".into(),
                    "Can reconstruct system behavior from log alone.".into(),
                ],
                code_template: String::new(),
                source_facts: vec!["py_system_event_logging".into()],
            },

            GeneratedSkill {
                id: "skill_staged_boot".into(),
                name: "Build Staged Boot Sequence".into(),
                description: "Implement a 7-stage boot pipeline with trust gating. Agent has situational awareness before the user's first prompt.".into(),
                category: PatternCategory::ConfigHierarchy,
                trigger: "When your agent needs to validate credentials, scan workspace, and load registries before accepting input.".into(),
                steps: vec![
                    "Define BootstrapGraph with ordered stages.".into(),
                    "Stage 1: Parallel prefetch (credentials, project scan, MDM).".into(),
                    "Stage 2: Environment guards and warnings.".into(),
                    "Stage 3: CLI parsing + trust decision.".into(),
                    "Stage 4: Parallel load — workspace context + command/tool registries.".into(),
                    "Stage 5: Trust-gated deferred init — plugins, skills, MCP, hooks ONLY if trusted.".into(),
                    "Stage 6: Mode routing (local/remote/ssh/teleport).".into(),
                    "Stage 7: Main loop (query engine).".into(),
                    "Gate each stage on the previous. Fail fast on critical errors.".into(),
                ],
                verification: vec![
                    "All stages execute in order.".into(),
                    "Untrusted mode skips stage 5 (plugins, skills, MCP).".into(),
                    "Prefetch runs in parallel.".into(),
                    "Agent ready to answer before first user prompt.".into(),
                ],
                code_template: String::new(),
            ...
            GeneratedSkill {
                id: "skill_doctor_pattern".into(),
                name: "Build Doctor Health Check".into(),
                description: "Unified /doctor command that validates API creds, connections, config, tools, and MCP servers. Run at startup and expose as admin command.".into(),
                category: PatternCategory::ConfigHierarchy,
                trigger: "When your agent needs self-diagnosis capability, or when debugging environment issues.".into(),
                steps: vec![
                    "Define HealthCheck result struct with per-component pass/fail.".into(),
                    "Check: API credentials valid and not expired.".into(),
                    "Check: External connections reachable (MCP servers, databases).".into(),
                    "Check: Config files parseable and consistent.".into(),
                    "Check: Tool registry complete (loaded vs expected).".into(),
                    "Check: Session store writable.".into(),
                    "Return structured results. Render as human-readable report.".into(),
                    "Run at startup automatically. Expose as /doctor slash command.".into(),
                ],
                verification: vec![
                    "Missing API key detected and reported.".into(),
                    "Unreachable MCP server reported with error.".into(),
                    "Corrupt config detected before first turn.".into(),
                ],
                code_template: String::new(),
            ...
            GeneratedSkill {
                id: "skill_token_budget".into(),
                name: "Implement Pre-Turn Token Budget".into(),
                description: "Track token usage with pre-turn budget checking. Stop BEFORE overspending. Expose budget status to users.".into(),
                category: PatternCategory::CostTracking,
                trigger: "When your agent makes API calls that cost money and needs cost controls.".into(),
                steps: vec![
                    "Define QueryEngineConfig: max_turns, max_budget_tokens, compact_after_turns.".into(),
                    "Track UsageSummary: input_tokens, output_tokens per message.".into(),
                    "Before each turn: project cost → check against budget → stop if over.".into(),
                    "Define stop_reason taxonomy: completed, max_budget_reached, max_turns_reached.".into(),
                    "Implement auto-compact with simple list slicing (not complex retry).".into(),
                    "Expose budget status in streaming events and session info.".into(),
                ],
                verification: vec![
                    "Budget exceeded → graceful stop before API call.".into(),
                    "Auto-compact triggers at threshold and doesn't loop infinitely.".into(),
                    "Stop reason included in final streaming event.".into(),
                    "User can see remaining budget.".into(),
                ],
                code_template: String::new(),
            ...
            GeneratedSkill {
                id: "skill_tool_pool_assembly".into(),
                name: "Build Tool Pool Assembly".into(),
                description: "Assemble per-session tool sets via 3-layer filtering: mode flags, feature flags, permission deny-lists. Fewer tools = better security + lower cost.".into(),
                category: PatternCategory::ToolSystem,
                trigger: "When different contexts need different tool sets (sub-agents, read-only mode, untrusted environments).".into(),
                steps: vec![
                    "Layer 1: Mode flags — simple_mode restricts to safe subset.".into(),
                    "Layer 2: Feature flags — include_mcp toggles MCP tool visibility.".into(),
                    "Layer 3: Permission context — deny_names (frozenset, O(1)) + deny_prefixes (tuple, prefix match).".into(),
                    "Compose layers in order: mode → features → deny-list.".into(),
                    "Return frozen ToolPool with assembled tools.".into(),
                    "Use the assembled pool to generate the tools parameter for API calls.".into(),
                ],
                verification: vec![
                    "Simple mode only exposes safe tools.".into(),
                    "Deny-list blocks exact names and prefixes.".into(),
                    "Sub-agents get appropriately restricted pools.".into(),
                ],
                code_template: String::new(),
                source_facts: vec!["py_tool_pool_assembly".into()],
            },

            GeneratedSkill {
                id: "skill_memory_system".into(),
                name: "Build File-Based Memory with Provenance".into(),
                description: "Implement an 8-module memory system with relevance scoring, aging, team scoping, and provenance tracking. Memory without provenance is accumulated hallucination.".into(),
                category: PatternCategory::SessionManagement,
                trigger: "When your agent needs to remember things across sessions with proper trust and freshness tracking.".into(),
                steps: vec![
                    "Define memory types: personal, team, project scope.".into(),
                    "Store each memory with: content, source, is_user_stated, last_validated, is_contradicted.".into(),
                    "Implement relevance scoring for retrieval queries.".into(),
                    "Add aging/TTL — stale memories score lower.".into(),
                    "Implement team isolation via scoped paths.".into(),
                    "Build prompt injection templates for memory augmentation.".into(),
                    "Never inject unscored/unvalidated memories into system prompt.".into(),
                    "Add directory scanning for memory discovery.".into(),
                ],
                verification: vec![
                    "Memories persist across sessions.".into(),
                    "Stale memories age out of relevance results.".into(),
                    "Team-scoped memories isolated from personal.".into(),
                    "Provenance tracked for every memory entry.".into(),
                ],
                code_template: String::new(),
