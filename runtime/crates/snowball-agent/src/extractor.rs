...
impl SourceExtractor {
    ...
    pub fn to_knowledge_base(&self, result: &ExtractionResult) -> KnowledgeBase {
        ...
        // Cross-reference: link structs whose fields reference other known structs
        let known_struct_names: Vec<String> = result
            .structs
            .iter()
            .map(|s| s.name.clone())
            .collect();
        for struct_def in &result.structs {
            let self_id = format!("struct_{}", struct_def.name.to_lowercase());
            for field in &struct_def.fields {
                for known in &known_struct_names {
                    if known != &struct_def.name
                        && (field.to_lowercase().contains(&known.to_lowercase())
                            || known.to_lowercase().contains(&field.to_lowercase()))
                    {
                        let other_id = format!("struct_{}", known.to_lowercase());
                        kb.add_relation(&self_id, &other_id);
                    }
                }
            }
        }

        // Cross-reference: link structs in same file (co-located = related)
        let mut structs_by_file: std::collections::HashMap<String, Vec<String>> =
            std::collections::HashMap::new();
        for struct_def in &result.structs {
            structs_by_file
                .entry(struct_def.file.display().to_string())
                .or_default()
                .push(format!("struct_{}", struct_def.name.to_lowercase()));
        }
        for (_, ids) in &structs_by_file {
            if ids.len() > 1 && ids.len() <= 8 {
                // Only link co-located structs in reasonably-sized files
                for i in 0..ids.len() {
                    for j in (i + 1)..ids.len() {
                        kb.add_relation(&ids[i], &ids[j]);
                    }
                }
            }
        }

        // Cross-reference: link enums to structs in same category
        for enum_def in &result.enums {
            let enum_id = format!("enum_{}", enum_def.name.to_lowercase());
            let enum_cat = self.categorize_enum(&enum_def.name);
            for struct_def in &result.structs {
                let struct_cat = self.categorize_struct(&struct_def.name);
                if enum_cat == struct_cat && enum_cat != PatternCategory::Other {
                    let struct_id = format!("struct_{}", struct_def.name.to_lowercase());
                    kb.add_relation(&enum_id, &struct_id);
                }
            }
        }
    ...
    fn assess_struct_impact(&self, name: &str) -> ImpactLevel {
        match name {
            // Only the core runtime struct is a gamechanger, not its Error/Config subtypes
            "ConversationRuntime" => ImpactLevel::Gamechanger,
            n if n.contains("Error") || n.contains("Input") || n.contains("Output") => ImpactLevel::Medium,
            n if n.contains("Config") && n.len() > 12 => ImpactLevel::Medium, // Sub-configs like RuntimeFeatureConfig
            "RuntimeConfig" | "PermissionPolicy" | "McpServerManager" => ImpactLevel::High,
            n if n.contains("Session") && !n.contains("Path") => ImpactLevel::High,
            n if n.contains("Policy") => ImpactLevel::High,
            n if n.contains("Mcp") || n.contains("MCP") => ImpactLevel::High,
            "HookRunner" => ImpactLevel::High,
            n if n.contains("Hook") => ImpactLevel::Medium, // HookConfig etc.
            n if n.contains("Tool") && n.contains("Spec") => ImpactLevel::High,
            n if n.contains("Agent") && !n.contains("Input") => ImpactLevel::High,
    ...
    fn categorize_struct(&self, name: &str) -> PatternCategory {
        match name {
            n if n.contains("Runtime") || n.contains("Conversation") || n.contains("Turn") => PatternCategory::AgenticLoop,
            n if n.contains("Prompt") || n.contains("SystemPrompt") || n.contains("ProjectContext") => PatternCategory::AgenticLoop,
            n if n.contains("Tool") || n.contains("Spec") || n.contains("Registry") || n.contains("Manifest") => PatternCategory::ToolSystem,
            n if n.contains("Mcp") || n.contains("MCP") || n.contains("JsonRpc") || n.contains("Jsonrpc") => PatternCategory::McpProtocol,
            n if n.contains("Permission") || n.contains("Policy") || n.contains("Auth") || n.contains("OAuth") || n.contains("oauth") => PatternCategory::PermissionModel,
            n if n.contains("Session") || n.contains("Compact") || n.contains("Message") || n.contains("Content") => PatternCategory::SessionManagement,
            n if n.contains("Config") || n.contains("Setting") || n.contains("Bootstrap") => PatternCategory::ConfigHierarchy,
            n if n.contains("Hook") => PatternCategory::HookSystem,
            n if n.contains("Agent") || n.contains("Subagent") => PatternCategory::SubAgent,
            n if n.contains("Sse") || n.contains("Stream") || n.contains("Render") || n.contains("Markdown") || n.contains("Spinner") => PatternCategory::Streaming,
            n if n.contains("Error") || n.contains("Retry") || n.contains("Backoff") => PatternCategory::ErrorHandling,
            n if n.contains("Usage") || n.contains("Cost") || n.contains("Token") || n.contains("Pricing") => PatternCategory::CostTracking,
            n if n.contains("Command") || n.contains("Slash") || n.contains("Cli") || n.contains("Repl") => PatternCategory::ToolSystem,
            n if n.contains("File") || n.contains("Glob") || n.contains("Grep") || n.contains("Bash") || n.contains("Sandbox") => PatternCategory::ToolSystem,
            n if n.contains("Request") || n.contains("Response") || n.contains("Client") || n.contains("Api") => PatternCategory::AgenticLoop,
            n if n.contains("Patch") || n.contains("Diff") || n.contains("Edit") => PatternCategory::ToolSystem,
            n if n.contains("Notebook") || n.contains("Todo") || n.contains("Skill") => PatternCategory::ToolSystem,
            n if n.contains("Web") || n.contains("Fetch") || n.contains("Search") => PatternCategory::ToolSystem,
            n if n.contains("Color") || n.contains("Theme") || n.contains("Input") || n.contains("Line") => PatternCategory::Streaming,
            n if n.contains("Remote") || n.contains("Upstream") || n.contains("Proxy") => PatternCategory::McpProtocol,
